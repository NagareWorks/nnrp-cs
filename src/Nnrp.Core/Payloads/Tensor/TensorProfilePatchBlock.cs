...
namespace Nnrp.Core
{
    public readonly struct TensorProfilePatchBlock : IEquatable<TensorProfilePatchBlock>
    {
        public const int BlockLength = 4 * sizeof(uint);

        public TensorProfilePatchBlock(uint minWidth, uint minHeight, uint maxWidth, uint maxHeight)
        {
            MinWidth = minWidth;
            MinHeight = minHeight;
            MaxWidth = maxWidth;
        ...
        public static bool TryParse(ReadOnlySpan<byte> source, out TensorProfilePatchBlock block)
        {
            return TryParse(source, out block, out _);
        }

        public static bool TryParse(ReadOnlySpan<byte> source, out TensorProfilePatchBlock block, out NnrpParseError error)
        {
            block = default;
            ...
            block = new TensorProfilePatchBlock(minWidth, minHeight, maxWidth, maxHeight);
            return true;
        }

        public bool Equals(TensorProfilePatchBlock other)
        {
        ...
        {
            return obj is TensorProfilePatchBlock other && Equals(other);
        }
