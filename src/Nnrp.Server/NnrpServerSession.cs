...
namespace Nnrp.Server
{
    public sealed class NnrpServerSession : INnrpServerSession
    {
            ...
            out NnrpParseError error)
        {
            ...
            if (inbound.Header.MessageType != MessageType.FrameSubmit
                || inbound.Header.MetaLength != FrameSubmitMetadata.MetadataLength)
            {
                error = NnrpParseError.InvalidMessageLayout;
                return false;
            }

            if (!FrameSubmitMetadata.TryParse(inbound.Metadata.Span, strict: true, out var metadata, out error))
            {
                return false;
            }

            if (metadata.SubmitMode != SubmitMode.Inline
                || metadata.ObjectRefMask != 0
                || metadata.PayloadKindBitmap != PayloadKind.Tensor
                || metadata.PayloadFrameCount != 0)
            {
                error = NnrpParseError.InvalidMessageLayout;
                return false;
            }
            ...
            if (inbound.Body.Length < cameraBytes)
            {
                error = NnrpParseError.SourceTooShort;
                return false;
            }

            var cameraBlock = inbound.Body.Slice(0, cameraBytes);
            var cursor = cameraBytes;
            if ((tileIndexBytes > 0 || metadata.SectionCount > 0)
                && !TryValidateZeroPadding(inbound.Body, cameraBytes, out cursor, out error))
            {
                return false;
            }

            if (inbound.Body.Length < cursor || inbound.Body.Length - cursor < tileIndexBytes)
            {
                error = NnrpParseError.SourceTooShort;
                return false;
            }

            var tileIndexBlock = inbound.Body.Slice(cursor, tileIndexBytes);
            ...
            if (metadata.SectionCount > 0
                && !TryValidateZeroPadding(inbound.Body, nextOffset, out cursor, out error))
            {
                return false;
            }
            ...
            for (var index = 0; index < sections.Length; index++)
            {
                if (inbound.Body.Length < cursor)
                {
                    error = NnrpParseError.SourceTooShort;
                    return false;
                }

                if (!TensorSectionBlock.TryParse(inbound.Body.Slice(cursor), metadata.TileCount, out var section, out var sectionBytes, out error))
                {
                    return false;
                }
                ...
                {
                    if (!TryValidateZeroPadding(inbound.Body, nextOffset, out cursor, out error))
                    {
                        return false;
                    }
                }
            ...
            if (cursor != inbound.Body.Length)
            {
                error = NnrpParseError.InvalidMessageLayout;
                return false;
            }

            var submitMetadata = new FrameSubmitMetadata(
                ...
                cameraBytes: metadata.CameraBytes,
                tileIndexBytes: metadata.TileIndexBytes);
            var submitBodyLength = FrameSubmitMessageBodyLength(
                TensorSubmitBlock.BlockLength,
                metadata.CameraBytes,
                metadata.TileIndexBytes,
                sections);
            var submitHeader = new NnrpHeader(
                versionMajor: inbound.Header.VersionMajor,
                messageType: inbound.Header.MessageType,
                flags: inbound.Header.Flags,
                metaLength: FrameSubmitMessage.MetadataLength,
                bodyLength: submitBodyLength,
                sessionId: inbound.Header.SessionId,
                frameId: inbound.Header.FrameId,
                viewId: inbound.Header.ViewId,
                routeId: inbound.Header.RouteId,
                traceId: inbound.Header.TraceId);
            submit = new FrameSubmitMessage(submitHeader, submitMetadata, cameraBlock, tileIds, sections);
            return true;
        }
        ...
        private static uint FrameSubmitMessageBodyLength(
            int submitBlockBytes,
            uint cameraBytes,
            uint tileIndexBytes,
            ReadOnlySpan<TensorSectionBlock> sections)
        {
            var bodyLength = checked(submitBlockBytes + (int)cameraBytes);
            bodyLength = BinaryAlignment.AlignUp(bodyLength, 8);
            bodyLength = checked(bodyLength + (int)tileIndexBytes);
