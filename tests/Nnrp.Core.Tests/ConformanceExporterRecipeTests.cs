...
namespace Nnrp.Core.Tests
{
    ...
    {
        [Fact]
        public void BuildManifestJsonFromSharedPreview2RecipeMatchesExpectedShape()
        {
            var recipeManifestPath = ResolveSharedRecipeManifestPath();
            var manifestJson = Program.BuildManifestJson("nnrp-1-preview2", recipeManifestPath);

            using var document = JsonDocument.Parse(manifestJson);
            var root = document.RootElement;
            Assert.Equal("nnrp-1-preview2", root.GetProperty("protocol_version").GetString());
        ...
        public void BuildManifestJsonRejectsMissingRecipeManifestPath()
        {
            ...
            var error = Assert.Throws<ArgumentException>(() => Program.BuildManifestJson("nnrp-1-preview2", path));
        ...
        {
            var path = WriteTempRecipeManifest("{\"protocol_version\":\"nnrp-1-preview1\",\"vectors\":[]}");
        ...
        {
            var path = WriteTempRecipeManifest("{\"protocol_version\":\"nnrp-1-preview2\",\"vectors\":[{\"recipe_type\":\"unknown\",\"name\":\"bad\"}]}");
        ...
        private static string ResolveSharedRecipeManifestPath()
        {
            ...
            var candidates = new[]
            {
                Path.Combine(repoRoot, "nnrp-conformance-action", "protocol", "nnrp-1-preview2", "vectors", "semantic-vectors.json"),
                Path.GetFullPath(Path.Combine(repoRoot, "..", "nnrp-conformance", "protocol", "nnrp-1-preview2", "vectors", "semantic-vectors.json")),
            };
            ...
            throw new InvalidOperationException("Shared preview2 semantic vector recipe manifest is not available. Set NNRP_CONFORMANCE_RECIPE_MANIFEST or checkout nnrp-conformance beside this repo.");
        }
