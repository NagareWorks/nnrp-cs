...
- [ ] Choose the default managed delivery model for preview3: event queue, callback registration, or async stream.
- [ ] Keep backend selection behind `Nnrp.NativeBridge` so tests can run against managed fixtures and native artifacts.
...
- [ ] Define cancellation and disposal behavior when a managed task is cancelled while a native operation is active.
- [ ] Define CI-owned package layouts for NuGet-style server, NuGet-style client, and Unity-style client distribution.
- [ ] Map every supported native artifact to a deterministic NuGet `runtimes/<rid>/native` path.
