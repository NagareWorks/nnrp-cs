...
- [x] Lock the C# preview3 rollout onto the frozen Rust-owned protocol contract rather than reviving a second managed hot path.
...
- [x] Finalize the preview3 public C# surface as the next in-place `NNRP/1` update without retaining superseded preview-era shims.
  ...
  - [x] Remove or quarantine superseded preview-era helper families once the remaining managed diagnostic paths are isolated.

## FFI Consumption

- [x] Consume the frozen handle families for connection, session, operation, schema, cache lease, and buffer views.
  ...
  - [x] Wrap stable borrowed buffer-view handles once those handles are exposed by the bridge contract.
- [x] Implement callback, polling, and event-queue adapters according to the frozen Rust binding contract.
...
- [x] Implement `SESSION_OPEN` / `SESSION_OPEN_ACK`, explicit session-close, and recovery semantics exactly as frozen in `nnrp-doc`.
  ...
  - [x] Implement recovery semantics exactly as frozen in `nnrp-doc`.
...
- [x] Replace repo-staged Unity/DLL distribution assumptions with CI-published package definitions.
  ...
  - [x] Move final server/client/Unity distribution outputs behind CI-published package definitions.
- [x] Split package outputs into NuGet-style server dependency, NuGet-style client dependency, and Unity-style client dependency.
- [x] Keep GitHub Packages as the first distribution target while leaving room for later NuGet / UPM registry rollout.
